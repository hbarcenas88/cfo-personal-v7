...
function renderFilterPanel(filters, state) {
  return card(`
    <div class="metric-top">
      <h2 style="margin:0;font-size:19px;">Filtros de categorías</h2>
      <button class="chip" data-clear-cat-filters>Limpiar filtros</button>
    </div>
    ...
    <div class="chip-row">
      ${filters.categories.map(cat => `<button class="chip active" data-remove-cat-filter="${cat}">${cat} ${icon('x')}</button>`).join('')}
      ${state.categories.filter(cat => !filters.categories.includes(cat.name)).slice(0, 6).map(cat => `<button class="chip" data-add-cat-filter="${cat.name}">${cat.name}</button>`).join('')}
    </div>
  `);
}
...
function budgetLine(label, amount, cls = '', attr = '') {
  return `<div class="row-card" ${attr} style="grid-template-columns:1fr auto;min-height:54px;"><span><strong>${label}</strong>${attr ? '<small class="row-subtitle">Doble toque para auditar</small>' : ''}</span><strong class="row-amount ${cls}">${amount < 0 ? '-' : ''}${formatMoney(amount)}</strong></div>`;
}
...
function categoryCard(row, expanded) {
  ...
  return card(`
    <button class="row-card" style="grid-template-columns:auto 1fr auto;" data-cat-expand="${row.name}">
      ...
      <span class="row-amount ${over ? 'danger' : ''}">${formatMoney(row.spent)}<small style="display:block;font-size:12px;color:${over ? 'var(--red)' : 'var(--green)'}">${row.planned ? `${pct.toFixed(0)}%` : 'sin ppto'}</small></span>
    </button>
    <div class="progress"><span style="width:${row.planned ? Math.min(100, pct) : 100}%;background:${over ? 'var(--red)' : color}"></span></div>
    ${over ? `<div class="row-subtitle" style="margin-top:8px;">Exceso: <strong class="danger">${formatMoney(row.spent - row.planned)}</strong></div>` : ''}
